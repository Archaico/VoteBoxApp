...
class OfflineQueueService {
  ...
  async queueVote(voteData: {
    proposalId: string;
    choice: string;
    voterPubKey: string;    timestamp: number;
  }): Promise<void> {
    const queuedVote: QueuedVote = {
      id: `vote_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      proposalId: voteData.proposalId,
      choice: voteData.choice,
      voterPubKey: voteData.voterPubKey,      timestamp: voteData.timestamp,
      attempts: 0,
    };
  ...
  private async processItem(item: QueuedItem): Promise<void> {
    if (this.isVote(item)) {
      await blockchainService.submitVote({
        ...
        choice: item.choice,
        voterPubKey: item.voterPubKey,        timestamp: item.timestamp,
      });
    } else {
  ...
  private isVote(item: QueuedItem): item is QueuedVote {
    return 'voterPubKey' in item && 'choice' in item;
  }
}
